...
REM quitar el rem de alguna de estas líneas si queremos activar por defecto alguno de los argumentos:
...
set random_images_dir=f:\SPACE_IMAGES
	REM WEATHER:
set htmlTOpdf_exe="P:\mysync\projects\ocio_\login_space\wkhtmltoimage.exe"
set widget_address="P:\mysync\projects\ocio_\login_space\widget.html"
	REM IMAGEMAGICK:
set mogrify_exe="s:\imagemagick\mogrify.exe"
set composite_exe="s:\imagemagick\composite.exe"
set convert_exe="s:\imagemagick\convert.exe"
...
SET count=1
FOR /F "tokens=* USEBACKQ" %%F IN (`dir %random_images_dir% /b ^| wc -l ^| cut -d" " -f5`) DO ( SET total=%%F )
...
:NORAND
if NOT %arg2% == y (
	curl -k https://apod.nasa.gov/apod/astropix.html>%temp%\1.a
	for /f delims^=^"^ tokens^=2 %%# in ('type %temp%\1.a ^| grep "jpg" ^| head -n1') do (
	 curl -k https://apod.nasa.gov/%%#>%temp%\backgroundDefault.jpg
	)
	del %temp%\1.a
...
for /f "tokens=1 delims= " %%# in ('wc -c %temp%\backgroundDefault.jpg') do (
	if %%# LSS 5000 (
...
if %arg3%==y (
 ...
 %convert_exe% %temp%\extract.png -crop 182x265+8+65 %temp%\extract.png
...
if %arg1%==y (
	REM ---Windows 10---:
 w10bg.exe /I %temp%\backgroundDefault.jpg
) else (
 	REM ---Windows 7 ---:
